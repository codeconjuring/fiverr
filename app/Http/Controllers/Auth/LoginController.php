...
class LoginController extends Controller
{
    ...
    public function otpVerification(){
        ...
        $data['user_id']    = Session::get('registerUserId');
        $data['user_phone'] = Session::get('registerUserFormattedPhone');

        return view('frontend.auth.verification', $data);     
    }
    public function verifyOtp(Request $request){
        
       $user = User::where(['id' => $request->user_id, 'formattedPhone' => $request->user_phone])->first();
       ...
       if (!empty($user)) {
            ...
            if ($user->otp_code == $request->otp_code) {
               $user->otp_verified = 1;
               $user->update();
               $this->helper->one_time_message('success', __('Verification successfully completed, login now!'));
               return redirect('/login');
            } else {
                $this->helper->one_time_message('error', __('Verification code does not match!'));
                return back();
           }      
       }
    ...
    public function resendOtp(Request $request)
    {
        ...
        if(! empty($user->formattedPhone) ) {
            $updateotpCode = mt_rand(100000, 999999);
            $user->otp_code= $updateotpCode;
            $user->update();
            //sending otp verification code to mobile and email
            $otpMeaasge = "Your new verification code is: ".$updateotpCode;
            $otp_status = 'resend';
            $this->helper->otpVerification($user, $otpMeaasge, $otp_status);
            return back();
        }
    ...
    public function authenticate(Request $request)
    {
        ...
        {
            //Check User Status
            $checkLoggedInUser = User::where(['email' => $loginData['value']])->first(['id','status','otp_verified','formattedPhone','email']);
            ...
            if (checkOtpVerification() == 'Enabled') {
                ...
                if ( checkAppSmsEnvironment() ) {
                    ...
                    if( $checkLoggedInUser->otp_verified == 0 ) {

                        $otpCode = mt_rand(100000, 999999);
                       
                        $checkLoggedInUser->otp_code = $otpCode;
                        $checkLoggedInUser->update();

                        $otpMeaasge="Your verification code is: " . $otpCode;

                        if(!empty($checkLoggedInUser->formattedPhone))
                        {
                            $otpVerified = sendSMS($checkLoggedInUser->formattedPhone,$otpMeaasge);
                        }
                        

                        Session::put('registerUserId',$checkLoggedInUser->id);
                        Session::put('registerUserFormattedPhone',$checkLoggedInUser->formattedPhone);
                        $this->helper->one_time_message('error', __('You are not verified, check your phone for verification code!'));
