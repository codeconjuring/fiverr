...
class LoginController extends Controller
{
    ...
    {
        $user = User::where(['id' => $request->user_id, 'email' => $request->user_email])->first();
        if ($user->otp_code == $request->otp_code) {
            $user->otp_verified = 1;
            $user->update();
            $message = "Verification successfully completed, login now!";
            $statuscode = 200;
        } else {
    ...
    public function resendOtp(Request $request)
    {
        ...
        if ($user) {
            ...
            $otp_status = 'resend';
            $message = $this->helper->otpVerificationApi($user, $otpMeaasge, $otp_status);
        }
        $success['status']  = '200';
        $success['message'] = $message;
        return response()->json(['success' => $success]);
    ...
    public function login(Request $request)
    {
        ...
        $checkLoggedInUser = User::where(['email' => $request->email])->first();
        ...
        if ($checkLoggedInUser->otp_verified == 0) {
            ...
            $otp_status = 'login';
            $message = $this->helper->otpVerificationApi($checkLoggedInUser, $otpMeaasge, $otp_status);

            $success['id']  = $checkLoggedInUser->id;
            $success['email']  = $checkLoggedInUser->email;
            $success['status']      = 402;
            $success['message']     = $message;
