...
class CoinPaymentController extends Controller
{
    ...
    {
        // dd($serialize);
        $data['data']       = $data       = CoinPayment::get_payload($serialize); //dd($data);
        ...
        config(['coinpayment.default_currency' => $data['data']['payload']['currency']]);
            // dd(\Config::get(['coinpayment.default_currency']));
    ...
    public function trx_info(Request $req)
    {
        ...
        if (auth()->check())
        {
            //logged in
            $user            = auth()->user();
            $existTransCount = (int) $user->coinpayment_transactions()->where('payment_id', $req->result['txn_id'])->count('id');
            if ($payment['error'] == 'ok' && $existTransCount === 0)
            {
                $data    = $payment['result'];
                $payload = $req->payload;
                $saved = [
                ...
                if (isset($req->payload['type']) && $req->payload['type'] == "deposit") //deposit
                {
                    ...
                    try
                    {
                        ...
                        $payload['receivedf']      = $data['receivedf']; //Added by Aminul
                        $payload['time_expires']   = $data['time_expires']; //Added by Aminul
    ...
    public function successTransactionInfo(Request $req) {
        $user                   = auth()->user();
        $result                 = $user->coinpayment_transactions()->where('payment_id', $req->txn_id)->first();
        $payload                = json_decode($result->payload);
        $result['receivedf']    = $payload->receivedf;
        $result['time_expires'] = $payload->time_expires;
        // session()->forget(['coinpaymentAmount', 'wallet_currency_id', 'method', 'payment_method_id', 'amount', 'transInfo']);
        // clearActionSession();
        return $result;
    }
