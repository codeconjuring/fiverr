...
class Transaction extends Model
{
    ...
    public static $transactionTypes           = [Deposit, Withdrawal, Transferred, Received, Exchange_From, Exchange_To, Request_From, Request_To, Payment_Sent, Payment_Received, Crypto_Sent, Crypto_Received, Order_Product, Order_Received, Referral_Award, Voucher_Created, Voucher_Activated];
...
/*Start of relationships*/
    ...
    {
        return $this->belongsTo(Order::class, 'transaction_reference_id', 'id');
    }
...
//common functions - starts
    ...
    public function getTransactions($from, $to, $type, $wallet, $status)
    {
        ...
        if (!empty($type) && $type != 'all') {
            ...
            if ($type == Deposit || $type == Withdrawal) {
                ...
                } elseif ($type == 'order_product') {
                    $whereInCondition = [Order_Product];
                } elseif ($type == 'order_received') {
                    $whereInCondition = [Order_Received];
                } elseif ($type == 'referral_award') {
    ...
    public function getTransactionLists($type, $user_id)
    {
        ...
        for ($i = 0; $i < count($transaction); $i++) {
            ...
            if ($transaction[$i]->orders) {
                $transactions[$i]['product_image'] = $transaction[$i]->orders->product->photo;
            }
    ...
    public function getTransactionDetails($tr_id, $user_id)
    {
        ...
        if ($transaction->orders) {
            $transaction->product_name = @$transaction->orders->product->title;
            $transaction->category     = @$transaction->orders->product->product_category->name;
            $transaction->code         = @$transaction->orders->product->product_code;
            $transaction->shop         = @$transaction->orders->product->store->name;
        }
    ...
    public static function create($user_id = null, $end_user_id = null, $currency_id = null, $transaction_reference_id = null, $transaction_type_id = null, $uuid = null, $total = 0, $subtotal = 0, $status = null)
    {
