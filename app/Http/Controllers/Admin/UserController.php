...
use App\DataTables\Admin\EachUserReferralAwardsDataTable;
...
// Extended Functions (Crypto Send & Receive)- ends
    ...
    public function eachUserReferralAwards($id, EachUserReferralAwardsDataTable $dataTable)
    {
        $data['menu']     = 'users';
        $data['sub_menu'] = 'users_list';
        $data['users'] = $users = User::find($id);
        ...
        if (isset($_GET['btn']))
        {
            if (empty($_GET['from']))
            {
                $data['from'] = $from = null; //variable -from - For Each Currency Referral Award
                $data['to']   = $to   = null; //variable -to - For Each Currency Referral Award
            }
            else
            {
                $data['from'] = $from = $_GET['from']; //variable -from - For Each Currency Referral Award
                $data['to']   = $to   = $_GET['to']; //variable -to - For Each Currency Referral Award
            }
            $data['currency'] = $currency = $_GET['currency']; //variable -from - For Each Currency Referral Award
        }
        else
        {
            $data['from']     = $from     = null;
