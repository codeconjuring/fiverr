...
class CurrencyPaymentMethodController extends Controller
{
    ...
    public function updatePaymentMethodCredentials(Request $request)
    {
        ...
        if (empty($currencyPaymentMethod))
        {
            ...
            elseif (!empty($request->cib))
            {
                $currencyPaymentMethod->method_data = json_encode($request->cib);
                if (isset($request->cib_status) && $request->cib_status == 'Active')
                {
                    $currencyPaymentMethod->activated_for = json_encode(['deposit' => '']);
                }
                else
            ...
            elseif (!empty($request->dahabia))
            {
                $currencyPaymentMethod->method_data = json_encode($request->dahabia);
                if (isset($request->dahabia_status) && $request->dahabia_status == 'Active')
                {
                    $currencyPaymentMethod->activated_for = json_encode(['deposit' => '']);
                }
                else
