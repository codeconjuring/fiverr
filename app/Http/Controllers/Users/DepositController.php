...
class DepositController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        else
        {
            ...
            else if ($method == 'Cib')
            {
                ...
                $currencyCode = $currency->code;
                $currencyNumber = Currency::where(['code' => $currencyCode])->value('number');
                $orderNumber = uniqid();
                $language = 'en';
                if (in_array(Session::get('dflt_lang'),['ar', 'en', 'fr'])) {
                    $language = Session::get('dflt_lang');
                }

                $returnUrl = url('deposit/cib-deposit-store');
                $failUrl = url('deposit/payment_cancel');

                if ($methodData->mode == 'sandbox') {
                    $getFormUrl = 'https://test.satim.dz/payment/rest/register.do?currency='.$currencyNumber.'&amount='.$paymentAmount.'&language='.$language.'&orderNumber='.$orderNumber.'&userName='.$methodData->username.'&password='.$methodData->password.'&returnUrl='.$returnUrl.'&failUrl='.$failUrl.'&jsonParams={"force_terminal_id":"'.$methodData->terminal_id.'","udf1":"2018105301346","udf5":"ggsf85s42524s5uhgsf"}';
                } elseif ($methodData->mode == 'live') {
                    $getFormUrl = 'https://cib.satim.dz/payment/rest/register.do?currency='.$currencyNumber.'&amount='.$paymentAmount.'&language='.$language.'&orderNumber='.$orderNumber.'&userName='.$methodData->username.'&password='.$methodData->password.'&returnUrl='.$returnUrl.'&failUrl='.$failUrl.'&jsonParams={"force_terminal_id":"'.$methodData->terminal_id.'","udf1":"2018105301346","udf5":"ggsf85s42524s5uhgsf"}';
                }

                $curl = curl_init();
                
                curl_setopt_array($curl, array(
                    CURLOPT_URL => $getFormUrl,
                    ...
                    'Cookie: cookiesession1=678A8C36EFGHIJKLMNOPQRSTUV01B60E'
                    ),
                ));
                ...
                if ( isset($responseData->orderId) && $responseData->errorCode === '0' && isset($responseData->formUrl)) {
                    Session::put('cib_order_id', $responseData->orderId);
                    Session::put('order_number', $orderNumber);
                    Session::put('language', $language);
                    return redirect($responseData->formUrl);
                }
            }
            ...
            else if ($method == 'Dahabia')
            {
                ...
                $returnUrl = url('deposit/dahabia-deposit-store');
                $failUrl = url('deposit/payment_cancel');

                if ($methodData->mode == 'sandbox') {
                    $getFormUrl = 'https://test.satim.dz/payment/rest/register.do?currency='.$currencyNumber.'&amount='.$paymentAmount.'&language='.$language.'&orderNumber='.$orderNumber.'&userName='.$methodData->username.'&password='.$methodData->password.'&returnUrl='.$returnUrl.'&failUrl='.$failUrl.'&jsonParams={"force_terminal_id":"'.$methodData->terminal_id.'","udf1":"2018105301346","udf5":"ggsf85s42524s5uhgsf"}';
                } elseif ($methodData->mode == 'live') {
                    $getFormUrl = 'https://dahabia.satim.dz/payment/rest/register.do?currency='.$currencyNumber.'&amount='.$paymentAmount.'&language='.$language.'&orderNumber='.$orderNumber.'&userName='.$methodData->username.'&password='.$methodData->password.'&returnUrl='.$returnUrl.'&failUrl='.$failUrl.'&jsonParams={"force_terminal_id":"'.$methodData->terminal_id.'","udf1":"2018105301346","udf5":"ggsf85s42524s5uhgsf"}';
                }
                ...
                if ( isset($responseData->orderId) && $responseData->errorCode === '0' && isset($responseData->formUrl)) {
                    Session::put('dahabia_order_id', $responseData->orderId);
                    Session::put('order_number', $orderNumber);
                    Session::put('language', $language);
                    return redirect($responseData->formUrl);
                }
    ...
    public function cibDepositStore(Request $request)
    {
        $requestCibOrderId = $request['orderId'];
        $cibOrderId = Session::get('cib_order_id');
        $language = Session::get('language');

        if ($cibOrderId == $requestCibOrderId) {

            $methodId = Session::get('payment_method_id');
            $currencyId = session('wallet_currency_id');
            ...
            if ($methodData->mode == 'sandbox') {
                $paymentResponseUrl = 'https://test.satim.dz/payment/rest/confirmOrder.do?language='.$language.'&orderId='.$cibOrderId.'&password='.$methodData->password.'&userName='.$methodData->username;
            } elseif ($methodData->mode == 'live') {
                $paymentResponseUrl = 'https://cib.satim.dz/payment/rest/confirmOrder.do?language='.$language.'&orderId='.$cibOrderId.'&password='.$methodData->password.'&userName='.$methodData->username;
            }

            $curl = curl_init();

            curl_setopt_array($curl, array(
            CURLOPT_URL => $paymentResponseUrl,
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_ENCODING => '',
                CURLOPT_MAXREDIRS => 10,
                CURLOPT_TIMEOUT => 0,
            ...
            if ($responseData->ErrorMessage == 'Success' && $responseData->ErrorCode === '0' &&  Session::get('order_number') ==  $responseData->OrderNumber) {
                ...
                try
                {
                    ...
                    return \Redirect::route('deposit.cib.success')->with(['data' => $data]);
                }
                catch (Exception $e)
                {
                    DB::rollBack();
                    Session::forget(['coinpaymentAmount', 'wallet_currency_id', 'method', 'payment_method_id', 'amount', 'publishable', 'transInfo', 'order_number', 'cib_order_id']);
                    clearActionSession();
                ...
                session()->forget(['order_number', 'cib_order_id', 'wallet_currency_id', 'method', 'payment_method_id', 'amount', 'order_number', 'cib_order_id']);
                if ($responseData->ErrorCode === '1') {
                    $this->helper->one_time_message('error', 'Order ID not found.');
                    return redirect('deposit');
                } elseif ($responseData->ErrorCode === '2') {
                    $this->helper->one_time_message('error', 'Order already confirmed.');
                    return redirect('deposit'); 
                } elseif ($responseData->ErrorCode === '3') {
                    $this->helper->one_time_message('error', 'Access denied.');
                    return redirect('deposit');
                } elseif ($responseData->ErrorCode === '5') {
                    $this->helper->one_time_message('error', 'Access denied.');
                    return redirect('deposit');
                } elseif ($responseData->ErrorCode === '6') {
                    $this->helper->one_time_message('error', 'Unknown order ID.');
                    return redirect('deposit');
                } elseif ($responseData->ErrorCode === '7') {
                    $this->helper->one_time_message('error', 'System error.');
                    return redirect('deposit');
                }
            } 
            ...
            session()->forget(['order_number', 'cib_order_id', 'wallet_currency_id', 'method', 'payment_method_id', 'amount', 'order_number', 'cib_order_id']);
            $this->helper->one_time_message('error', 'Payment not completed.');
            return redirect('deposit');
        }

    }

    public function cibDepositSuccess()
    {
        if (empty(session('data'))) {
            ...
            //clearing session
            session()->forget(['coinpaymentAmount', 'wallet_currency_id', 'method', 'payment_method_id', 'amount', 'publishable', 'transInfo', 'data', 'order_number', 'cib_order_id']);
            clearActionSession();
            return view('user_dashboard.deposit.success', $data);
        }
    }

    public function dahabiaDepositStore(Request $request)
    {
        $requestDahabiaOrderId = $request['orderId'];
        $dahabiaOrderId = Session::get('dahabia_order_id');
        $language = Session::get('language');

        if ($dahabiaOrderId == $requestDahabiaOrderId) {
            ...
            if ($methodData->mode == 'sandbox') {
                $paymentResponseUrl = 'https://test.satim.dz/payment/rest/confirmOrder.do?language='.$language.'&orderId='.$dahabiaOrderId.'&password='.$methodData->password.'&userName='.$methodData->username;
            } elseif ($methodData->mode == 'live') {
                $paymentResponseUrl = 'https://dahabia.satim.dz/payment/rest/confirmOrder.do?language='.$language.'&orderId='.$dahabiaOrderId.'&password='.$methodData->password.'&userName='.$methodData->username;
            }
            ...
            if ($responseData->ErrorMessage == 'Success' && $responseData->ErrorCode === '0' &&  Session::get('order_number') ==  $responseData->OrderNumber) {
                ...
                try
                {
                    ...
                    $data['transaction'] = $transaction;
                    return \Redirect::route('deposit.dahabia.success')->with(['data' => $data]);
                }
                catch (Exception $e)
                {
                    DB::rollBack();
                    Session::forget(['coinpaymentAmount', 'wallet_currency_id', 'method', 'payment_method_id', 'amount', 'publishable', 'transInfo', 'order_number', 'dahabia_order_id']);
                    clearActionSession();
                    $this->helper->one_time_message('error', $e->getMessage());
                    return back();
                }
    ...
    public function dahabiaDepositSuccess()
    {
        if (empty(session('data'))) {
            return redirect('deposit');
        } else {
            $data['transaction'] = session('data')['transaction'];
            //clearing session
            session()->forget(['coinpaymentAmount', 'wallet_currency_id', 'method', 'payment_method_id', 'amount', 'publishable', 'transInfo', 'data', 'order_number', 'dahabia_order_id']);
            clearActionSession();
            return view('user_dashboard.deposit.success', $data);
        }
    }
